...
Ask: "Pick a number to dive in, or tell me what you want to focus on."
...
For **Discussion** or other types:
```bash
gh api repos/OWNER/REPO/notifications --jq '.[] | select(.id == "NOTIF_ID")'
```

After loading context, **summarize** the situation in 3-5 sentences:
- What's happening (PR change summary, issue description, comment thread)
...
- "Mark all as read": `gh api -X PUT /notifications`
